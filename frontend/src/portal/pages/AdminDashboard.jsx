...
const TABS = ['Users', 'Create vendor / customer', 'Vendor applications']
...
export default function AdminDashboard () {
  ...
  const usersQuery = useQuery({
    ...
    queryFn: () => portalApi.get('/api/admin/users').then(r => r.data.users)
  })
  const applicationsQuery = useQuery({
    queryKey: ['admin', 'vendor-applications'],
    queryFn: () => portalApi.get('/api/admin/vendor-applications').then(r => r.data.applications)
  })

  const users = usersQuery.data ?? []
  const applications = applicationsQuery.data ?? []
  const loading = usersQuery.isLoading || applicationsQuery.isLoading
  const refresh = () => queryClient.invalidateQueries({ queryKey: ['admin'] })

  return (
    <PortalLayout title={user?.name ? `${user.name} — Admin` : 'Admin'}>
      ...
      {(usersQuery.error || applicationsQuery.error) && (
        <p className='mb-4 text-sm font-semibold text-rose-600'>
          {(usersQuery.error || applicationsQuery.error).message}
        </p>
      )}
      ...
      {loading
        ...
        : (
          <>
            ...
            {tab === 'Create vendor / customer' && <CreateUserTab onCreated={refresh} />}
...
function UsersTab ({ users, onChanged }) {
  const toggleStatus = async user => {
    ...
    try {
      await portalApi.patch(`/api/admin/users/${user._id}/status`, { status })
  ...
  if (!users.length) return <p className='text-sm text-royal-900/50'>No vendors or customers yet.</p>
    ...
    <Card>
      <div className='overflow-x-auto'>
        <table className='w-full text-left text-sm'>
          ...
          <tbody>
            {users.map(u => (
              <tr key={u._id} className='border-b border-royal-50'>
                <td className='py-2.5 pr-4 font-semibold text-royal-950'>{u.name}</td>
                <td className='py-2.5 pr-4 text-royal-900/70'>{u.email}</td>
                <td className='py-2.5 pr-4 capitalize text-royal-900/70'>{u.role}</td>
                <td className='py-2.5 pr-4'>
                  <span className={`rounded-full px-2.5 py-1 text-xs font-bold ${u.status === 'active' ? 'bg-olive-50 text-olive-800' : 'bg-rose-50 text-rose-700'}`}>
                    {u.status}
                  </span>
                </td>
                <td className='py-2.5 pr-4'>
                  <button onClick={() => toggleStatus(u)} className='text-xs font-bold text-royal-700 hover:underline'>
...
function CreateUserTab ({ onCreated }) {
  const {
    ...
    defaultValues: { name: '', email: '', role: 'vendor' },
    mode: 'onTouched'
  })

  const submit = async values => {
    try {
      const { data } = await portalApi.post('/api/admin/users', values)
...
function VendorApplicationsTab ({ applications, onChanged }) {
  const decide = async (userId, decision) => {
    try {
      await portalApi.patch(`/api/admin/vendor-applications/${userId}`, { decision })
        ...
        <Card key={app._id}>
          <div className='flex flex-wrap items-start justify-between gap-3'>
            <div>
              ...
              <p className='text-sm text-royal-900/60'>Applying as "{app.vendorApplication.businessName}"</p>
